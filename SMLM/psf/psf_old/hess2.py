...
def hessian2(x, y, x0, y0, sigma, N0, B0):
    ...
    h_NB = np.zeros_like(h_xx)
    h_Bx = np.zeros_like(h_xx)
    h_By = np.zeros_like(h_xx)
    h_Bs = np.zeros_like(h_xx)
    h_BN = np.zeros_like(h_xx)
    h_BB = np.zeros_like(h_xx)
    H = np.array([[h_xx,h_xy,h_xs,h_xN,h_xB], [h_yx,h_yy,h_ys,h_yN,h_yB], [h_sx,h_sy,h_ss,h_sN,h_sB],[h_Nx,h_Ny,h_Ns,h_NN,h_NB],[h_Bx,h_By,h_Bs,h_BN,h_BB]], dtype=np.float64)
    return H
