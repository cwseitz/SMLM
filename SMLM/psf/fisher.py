...
def hessian(theta,adu,eta,texp,gain,var):
    lx, ly = adu.shape
    x0,y0,sigma,N0,B0 = theta
    X,Y = np.meshgrid(np.arange(0,lx),np.arange(0,ly))
    J1 = jacobian1(X,Y,x0,y0,sigma,N0,B0,eta,texp,gain,var)
    H1 = hessian1(adu,X,Y,x0,y0,sigma,N0,B0,eta,texp,gain,var)
    J2 = jacobian2(adu,X,Y,x0,y0,sigma,N0,B0,eta,texp,gain,var)
    H2 = hessian2(X,Y,x0,y0,sigma,N0,B0,eta,texp,gain,var)
...
def hessian_batch(theta,adu,eta,texp,gain,var):
    lx, ly = adu.shape
    ntheta, ns = theta.shape
    Hbatch = np.zeros((ns,ntheta,ntheta))
    for n in range(ns):
        x0,y0,sigma,N0,B0 = theta[:,n]
        X,Y = np.meshgrid(np.arange(0,lx),np.arange(0,ly))
        J1 = jacobian1(X,Y,x0,y0,sigma,N0,B0,eta,texp,gain,var)
