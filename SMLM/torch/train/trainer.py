    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(self.data_loader):
            ...
            if batch_idx % self.log_step == 0:
                self.writer.add_image('input', make_grid(data.cpu(), nrow=5, normalize=True))
                self.writer.add_image('output', make_grid(output.cpu(), nrow=5, normalize=True))
                self.writer.add_image('target', make_grid(target.cpu(), nrow=5, normalize=True))
